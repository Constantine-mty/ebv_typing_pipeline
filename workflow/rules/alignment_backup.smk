...
rule align_to_human:
    ...
    output:
        unmapped_bam = "results/alignment/{sample}_human_unmapped.bam",
        flagstat = "results/alignment/{sample}_human_flagstat.txt",
    params:
    ...
    shell:
        ...
        if [ -f "{input.r2}" ]; then
            ...
            hisat2 {params.extra} \
                ...
                --un-conc-gz results/alignment/{wildcards.sample}_human_unmapped_%.fq.gz \
                --summary-file {log} \
                2> {log}.err \
                | samtools view -bS - > /dev/null

            # Re-align unmapped reads to produce a BAM for QC
            hisat2 {params.extra} \
                -x {params.human_fa} \
                -p {threads} \
                -1 {input.r1} -2 {input.r2} \
                2>> {log}.err \
                | samtools view -bS -f 4 -F 256 - > {output.unmapped_bam}
        else
            ...
            hisat2 {params.extra} \
                ...
                -p {threads} \
                -U {input.r1} \
                --un-gz results/alignment/{wildcards.sample}_human_unmapped_S.fq.gz \
                --summary-file {log} \
                2> {log}.err \
                | samtools view -bS - > /dev/null
        ...
        samtools flagstat {output.unmapped_bam} > {output.flagstat}
        """


rule extract_unmapped_fastq:
    """Extract unmapped reads as FASTQ for EBV alignment."""
    input:
        unmapped_bam = "results/alignment/{sample}_human_unmapped.bam",
    output:
    ...
    shell:
        r"""
        name_sorted="results/alignment/{wildcards.sample}_human_unmapped.name_sorted.bam"
        samtools sort -n -@ {threads} -o "$name_sorted" {input.unmapped_bam}

        # Extract unmapped reads as FASTQ (handles both PE and SE)
        # Check if BAM has paired reads
        PAIRED=$(samtools view -c -f 1 "$name_sorted")
        if [ "$PAIRED" -gt 0 ]; then
            echo "Extracting paired-end unmapped reads..."
            samtools fastq -@ {threads} \
                -1 {output.r1} \
                -2 {output.r2} \
                -0 /dev/null \
                -s results/alignment/{wildcards.sample}_ebv_input_singletons.fastq.gz \
                -n "$name_sorted"
        else
            echo "Extracting single-end unmapped reads..."
            samtools fastq -@ {threads} -0 {output.r1} "$name_sorted"
            touch {output.r2}
        fi
        """
